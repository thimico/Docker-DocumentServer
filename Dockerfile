FROM thimico/ubuntubase

ENV LANG=pt_BR.UTF-8 LANGUAGE=pt_BR:en LC_ALL=pt_BR.UTF-8 DEBIAN_FRONTEND=noninteractive

RUN echo "#!/bin/sh\nexit 0" > /usr/sbin/policy-rc.d && \
    apt-get -y update && \
    apt-get  -yq install wget apt-transport-https curl p7zip-full && \
